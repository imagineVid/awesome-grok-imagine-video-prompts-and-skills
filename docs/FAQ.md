# FAQ

## What is this repository?

It is an ImagineVid-maintained collection of source-verifiable Grok Imagine Video prompts, playable results, and production notes. Every published entry must retain its creator, canonical source, model evidence, and playable result.

## What can Grok Imagine Video do?

xAI documents text-to-video and image-to-video generation, native video-audio output, motion and camera control, and video editing workflows such as restyling or adding and removing scene elements. Grok Imagine Video 1.5 is generally available in the xAI API as `grok-imagine-video-1.5`; xAI describes stronger motion, physics, audio, speech, and faster generation than the previous model.

Official references:

- [Grok Imagine Video 1.5 release](https://x.ai/news/grok-imagine-video-1-5)
- [Grok Imagine API overview](https://x.ai/news/grok-imagine-api)
- [Grok Imagine Video model documentation](https://docs.x.ai/developers/models/grok-imagine-video)

## Where can I use the model?

The collection's primary workflow link is [Grok Imagine on ImagineVid](https://imaginevid.io/grok-imagine). The current version-specific route is [Grok Imagine 1.5 on ImagineVid](https://imaginevid.io/grok-imagine-1-5).

## How are prompts selected?

Community cases enter the collection only after source verification, model confirmation, video review, workflow classification, and duplicate checks. Showcase-only posts and prompts without attributable results are excluded.

## How does localization work?

English remains the source language. Each localized README translates the editorial guidance and provides a reviewed prompt translation while keeping the creator's original English prompt visible for provenance and reuse.

## What makes a video prompt reusable?

A reusable brief usually names the subject and action, gives the camera a path, defines pacing, states the intended sound or dialogue, and says what must remain stable. Image-to-video prompts should separate anchor-frame constraints from motion instructions.
