/**
 * [INPUT]: 依赖 twitterapi.io 候选字段、本地视频提示词指纹与 Grok Imagine Video 模型证据
...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  if (
    ...
    hardFailures.push("missing verified Grok Imagine Video model evidence");
  }
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  const hasGrokImagineEvidence = /\bgrok[-\s]*imagine(?:[-\s]*video)?(?:[-\s]*1\.5)?\b|\bgrok-imagine-video(?:-1\.5)?\b/i.test(
    relevanceText
  );
  const namesDifferentVideoModel = /\b(?:veo|kling|seedance|sora|hailuo|runway|luma)\b/i.test(
    relevanceText
  ) && !hasGrokImagineEvidence;
  if (hasPromptSignal) {
    score += 30;
  ...
  if (!hasGrokImagineEvidence) {
    reasons.push("missing explicit Grok Imagine Video evidence; manual review required");
