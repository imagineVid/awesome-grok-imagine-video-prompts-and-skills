<a href="https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills">
  <img src="public/images/grok-imagine-video-prompt-skills-cover.png" alt="Grok Imagine Video Prompt Skills" width="100%" />
</a>

> Grok Imagine Video를 위한 출처 검증형 샷 브리프, 모션 패턴, 시청각 워크플로 라이브러리입니다.
# Awesome Grok Imagine Video 프롬프트 및 스킬

[![Awesome](https://awesome.re/badge.svg)](https://github.com/sindresorhus/awesome)
[![GitHub stars](https://img.shields.io/github/stars/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills?style=social)](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills)
[![License: CC BY 4.0](https://img.shields.io/badge/License-CC%20BY%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/)
[![Update README](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills/actions/workflows/update-readme.yml/badge.svg)](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills/actions)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](docs/CONTRIBUTING.md)

> 브리프를 연구하고 결과를 확인하며 제작자를 추적하세요. 표면적인 스타일을 복사하는 대신 연출 논리를 재사용하세요.

> **출처 표시 및 수정:** 게시된 각 사례는 제작자와 정식 출처로 연결됩니다. 권리는 각 소유자에게 있습니다. 출처 표시 변경이나 삭제는 issue를 열어 요청하세요.

---

[![English](https://img.shields.io/badge/English-Click%20to%20View-lightgrey)](README.md) [![Español](https://img.shields.io/badge/Espa%C3%B1ol-Click%20to%20View-lightgrey)](README.es.md) [![Português](https://img.shields.io/badge/Portugu%C3%AAs-Click%20to%20View-lightgrey)](README.pt.md) [![Italiano](https://img.shields.io/badge/Italiano-Click%20to%20View-lightgrey)](README.it.md) [![Deutsch](https://img.shields.io/badge/Deutsch-Click%20to%20View-lightgrey)](README.de.md) [![Français](https://img.shields.io/badge/Fran%C3%A7ais-Click%20to%20View-lightgrey)](README.fr.md) [![العربية](https://img.shields.io/badge/%D8%A7%D9%84%D8%B9%D8%B1%D8%A8%D9%8A%D8%A9-Click%20to%20View-lightgrey)](README.ar.md) [![日本語](https://img.shields.io/badge/%E6%97%A5%E6%9C%AC%E8%AA%9E-Click%20to%20View-lightgrey)](README.ja.md) [![한국어](https://img.shields.io/badge/%ED%95%9C%EA%B5%AD%EC%96%B4-Current-brightgreen)](README.ko.md) [![中文](https://img.shields.io/badge/%E4%B8%AD%E6%96%87-Click%20to%20View-lightgrey)](README.zh.md)
[![Nederlands](https://img.shields.io/badge/Nederlands-Click%20to%20View-lightgrey)](README.nl.md) [![Русский](https://img.shields.io/badge/%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9-Click%20to%20View-lightgrey)](README.ru.md) [![Türkçe](https://img.shields.io/badge/T%C3%BCrk%C3%A7e-Click%20to%20View-lightgrey)](README.tr.md) [![Polski](https://img.shields.io/badge/Polski-Click%20to%20View-lightgrey)](README.pl.md)

---

## Grok Imagine Video로 만들기

**[ImagineVid에서 Grok Imagine 워크플로 열기](https://imaginevid.io/ko/grok-imagine)**

이 저장소에서 제작자의 지시와 그 결과로 나온 움직임을 비교하세요. 샷 문법을 새 클립에 적용할 준비가 되면 ImagineVid를 여세요.

인기는 근거가 아닙니다. 완전한 프롬프트와 유용한 비디오가 있는 낮은 참여도의 게시물이 재현 가능한 연출이 없는 바이럴 쇼케이스보다 우선될 수 있습니다.

| 제작 요구 | 근거 라이브러리 | ImagineVid 워크플로 |
|---------|--------------|---------------------|
| 사례 검토 | 프롬프트, 결과, 출처 | 생성 및 비교 |
| 탐색 | 저장소 텍스트 검색 | 워크플로 중심 탐색 |
| 생성 | - | Grok Imagine 열기 |
| 읽기 | GitHub 네이티브 Markdown | 브라우저 제작 작업 공간 |
| 비디오 워크플로 | - | 제작 필터 |


### 제작 워크플로별 탐색

- [**카메라 연출 및 샷 디자인**](#workflow-camera-direction-shot-design) - 프레이밍, 카메라 경로, 블로킹, 페이싱, 공개 순서, 전환을 중심으로 구성한 샷 브리프입니다.
- [**대사, 연기 및 네이티브 오디오**](#workflow-dialogue-performance-native-audio) - 대사, 연기, 분위기음, 음악 또는 동기화된 사운드가 장면을 이끄는 퍼포먼스 중심 프롬프트입니다.
- [**제품 모션 및 광고 스팟**](#workflow-product-motion-commercial-spots) - 제품, 오퍼, 의상, 요리, 기기 또는 브랜드 순간을 움직임의 중심에 두는 광고 클립입니다.
- [**이미지-투-비디오 및 주제 연속성**](#workflow-image-to-video-subject-continuity) - 정지 이미지를 애니메이션화하면서 인물의 정체성, 구도, 제품 형태 또는 스토리보드 레이아웃을 보존하는 이미지 기반 워크플로입니다.
- [**스타일라이즈드 모션 및 시각 효과**](#workflow-stylized-motion-visual-effects) - 변형, 시뮬레이션, 초현실적 물리, 그래픽 모션 또는 독특한 미디어 처리로 구동되는 효과 및 애니메이션 패턴입니다.
- [**비디오 편집, 스타일 변경 및 장면 제어**](#workflow-video-editing-restyling-scene-control) - 연속성을 보호하면서 기존 영상의 스타일을 바꾸고 확장, 추가, 제거, 교체하거나 장면의 방향을 바꾸는 워크플로입니다.

---

## 목차

- [Grok Imagine Video로 만들기](#grok-imagine-video로-만들기)
- [Grok Imagine Video란?](#grok-imagine-video란)
- [컬렉션 상태](#컬렉션-상태)
- [추천 비디오 프롬프트](#community-featured-prompts)
- [커뮤니티 비디오 프롬프트](#community-prompt-cases)
- [검증된 사례 기여하기](#검증된-사례-기여하기)
- [라이선스](#라이선스)
- [제작자 크레딧](#제작자-크레딧)
- [저장소 성장](#저장소-성장)

---

## Grok Imagine Video란?

**Grok Imagine Video**는 텍스트, 이미지, 원본 영상에서 짧은 비디오를 생성하고 편집하며 결과의 일부로 오디오도 만드는 xAI의 시스템입니다. 현재 `grok-imagine-video-1.5` 모델은 xAI API를 통해 일반적으로 사용할 수 있습니다. xAI는 이 릴리스가 움직임, 물리적 일관성, 음성, 음향 효과, 분위기, 생성 속도를 개선했다고 설명합니다.

실용적인 프롬프트 작성에서는 각 요청을 무드보드가 아니라 샷 브리프로 다루세요. 보이는 동작, 카메라의 동작, 비트의 타이밍, 사운드 계획, 안정적으로 유지해야 하는 요소를 명시하세요. 아래 사례는 제작자의 프롬프트를 결과 옆에 보여 주므로, 유용한 연출과 장식적인 문구를 구분할 수 있습니다.

- **텍스트나 프레임에서 시작** - 글로 쓴 장면에서 생성하거나 이미 구도를 담고 있는 이미지에 움직임을 부여하세요
- **관찰 가능한 움직임을 연출** - 각 동작의 블로킹, 운동량, 객체 간 상호작용, 물리적 결과를 설명하세요
- **시놉시스가 아니라 비트를 작성** - 타이밍과 공개 순서가 중요할 때 타임스탬프나 짧은 동작 시퀀스를 사용하세요
- **장면과 함께 사운드를 생성** - 오디오가 스토리텔링의 일부라면 대사, 분위기음, 음악 또는 효과음을 포함하세요
- **기존 영상을 편집** - 클립의 스타일을 바꾸고, 장면 조건을 변경하거나 보이는 요소를 추가, 제거, 교체하세요
- **연속성을 명시적으로 보호** - 변형되어서는 안 되는 얼굴, 제품의 형태, 레이아웃, 의상 또는 배경을 밝혀 두세요

**현재 참고 자료:** [xAI Video 1.5 release](https://x.ai/news/grok-imagine-video-1-5) · [xAI model documentation](https://docs.x.ai/developers/models/grok-imagine-video) · [Grok Imagine on ImagineVid](https://imaginevid.io/ko/grok-imagine)

### 프롬프트를 샷 템플릿으로 바꾸기

재사용 가능한 비디오 프롬프트는 장면 변수와 연출 논리를 분리합니다. 검증된 카메라 경로, 비트 구조, 사운드 계획, 연속성 규칙은 유지하면서 주제, 배경, 대사 또는 제품을 바꾸세요.

**템플릿 패턴:**
```
[DURATION + ASPECT RATIO]. [SUBJECT]가 [SETTING]에서 [VISIBLE ACTION]을 수행한다. Camera: [FRAMING + MOVE]. Beats: [TIMED ACTIONS]. Audio: [DIALOGUE + FOLEY + AMBIENCE]. Preserve: [IDENTITY / PRODUCT / LAYOUT]. Avoid: [FAILURE MODES].
```

동작 하나와 카메라 아이디어 하나로 시작하세요. 눈에 보이는 제작 요구를 해결할 때만 타이밍, 오디오, 보존 제약을 추가하고, 생성 간에는 한 번에 변수 하나만 바꾸세요.

---

## 컬렉션 상태

<div align="center">

| 컬렉션 필드 | 현재 값 |
|--------|-------|
| 검증된 사례 | **18** |
| 에디터 추천 | **4** |
| 생성 시각 | **2026년 7월 23일 목요일 오전 10시 20분 48초 UTC** |

</div>

---

<a id="community-featured-prompts"></a>

## 추천 비디오 프롬프트

> 재현성, 움직임의 명확성, 제작 유용성을 기준으로 선정

<a id="prompt-1"></a>

### #1: 빠른 우주선 디테일 몽타주와 하드 트랜지션

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### 이 워크플로가 중요한 이유

매초 새로운 우주선 디테일과 단호한 줌 동작을 배정하면서 페이드를 명시적으로 금지하는 간결한 다중 장면 페이싱 테스트입니다.

#### 현지화 프롬프트

```
초고속 비행. 다중 장면의 빠른 실사 영상을 만드세요. 매초 우주선의 서로 다른 디테일을 날카롭게 줌인하고 줌아웃하세요. 날카로운 장면 전환, 페이드 없음.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Ultra-fast flight. Create a multi-scene, fast-paced live-action video. Use sharp zoom-ins and zoom-outs on different details of the spacecraft every second. Sharp scene transitions, no fade.
```

</details>

#### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2044049267589816320/vid/avc1/1280x720/NSIhpjHgXuMxvVlN.mp4?tag=24"><img src="https://pbs.twimg.com/amplify_video_thumb/2044049267589816320/img/pMWIqfLwp11wbUs4.jpg" height="420" alt="빠른 우주선 디테일 몽타주와 하드 트랜지션 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2044049267589816320/vid/avc1/1280x720/NSIhpjHgXuMxvVlN.mp4?tag=24)**
</div>

#### 근거

- **제작자:** [Dogan Ural](https://x.com/doganuraldesign)
- **정식 출처:** [정식 출처](https://x.com/doganuraldesign/status/2044049325299204120)
- **게시일:** 2026년 4월 14일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-3"></a>

### #2: 대사와 사운드 디자인을 곁들인 슬로 모션 시리얼 폭발

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### 이 워크플로가 중요한 이유

광각 구도, 슬로 모션 입자, 말로 하는 반응, 구체적인 폴리, 음악 연출을 조율하는 유쾌한 아침 광고입니다.

#### 현지화 프롬프트

```
아늑한 레트로 다이너 스타일의 주방 안에서 촬영한 시네마틱 광각 샷입니다. 어린 소녀가 색색의 시리얼이 담긴 그릇 위로 몸을 기울이고, 우유가 극적인 슬로 모션으로 위쪽으로 폭발합니다. 밝은 시리얼 링들이 소녀의 얼굴 주변 공중에 떠 있습니다. 소녀는 눈을 크게 뜨고 입을 벌린 놀라움과 기쁨의 표정으로 카메라를 정면으로 바라봅니다. 따뜻한 아침 햇살이 블라인드 사이로 들어옵니다. 어안 렌즈 왜곡, 사실적인 조명, 높은 디테일, 역동적인 움직임, 유쾌한 시네마틱 분위기로 장면은 장난스럽고 마법 같으며 활기찬 느낌을 냅니다. 길이: 10초.

대사:  우와… 내 아침 식사가 정말 날고 있는 거야?

사운드 디자인:

갑작스러운 우유가 튀는 소리.
가볍게 시리얼이 흩어지는 소리.
부드러운 마법의 반짝임 효과음.
소녀의 놀란 숨소리.
재미있고 장난스러운 배경 음악.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
A cinematic wide angle shot inside a cozy retro diner style kitchen. A young girl leans over a bowl of colorful cereal as milk bursts upward in dramatic slow motion. Bright cereal loops float through the air around her face. She looks directly at the camera with wide eyes and an open-mouth expression of surprise and delight. Warm morning sunlight streams through the blinds. The scene has a playful, magical, energetic feel, with fisheye lens distortion, realistic lighting, high detail, dynamic motion, and a fun cinematic vibe. Duration: 10 seconds.

Dialogue:  Whoa… is my breakfast actually flying?

Sound Design:

Sudden milk splash.
Light cereal scattering sounds.
Soft magical sparkle effect.
Girl’s surprised gasp.
Fun playful background music.
```

</details>

#### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2069395437908643841/vid/avc1/1280x704/HpEN37WNo_J8Cw9v.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2069395437908643841/img/g-URoeS90azUiBUG.jpg" height="420" alt="대사와 사운드 디자인을 곁들인 슬로 모션 시리얼 폭발 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2069395437908643841/vid/avc1/1280x704/HpEN37WNo_J8Cw9v.mp4?tag=25)**
</div>

#### 근거

- **제작자:** [BMX](https://x.com/bmx_ai13)
- **정식 출처:** [정식 출처](https://x.com/bmx_ai13/status/2069395522088030511)
- **게시일:** 2026년 6월 23일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-5"></a>

### #3: 동기화된 슬랩스틱 오디오를 곁들인 원테이크 인형 싸움

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)
![Reference](https://img.shields.io/badge/Reference-Image%20Needed-orange)

#### 이 워크플로가 중요한 이유

시작 경기장을 고정하고 연속적인 반격을 연출하며 대사, 충격 동작, 관중 소음, 소품 폴리를 지정하는 이미지-투-비디오 액션 브리프입니다.

#### 현지화 프롬프트

```
10초 / 이미지-투-비디오 / 하나의 연속 샷 / 네이티브 오디오 / 16:9

첨부 이미지를 정확한 첫 프레임으로 사용하세요. PuppetFight 경기장, 핫핑크 링 로프, 소리 지르는 인형 관중, 거친 상부 스포트라이트, 거친 링 바닥, 폭발하는 솜, 혼란스러운 언더그라운드 싸움의 에너지를 보존하세요. 왼쪽에는 Tina Tornado가 공중에 떠서 헝클어진 머리와 날아가는 핑크 롤러를 한 채 망가진 접이식 의자를 움켜쥐고 있습니다. 오른쪽에서는 검은 선글라스를 쓴 해골 장갑의 펑크 복서가 그녀에게 잔혹한 펀치를 날리고, 천 조각과 먼지와 솜이 공중으로 폭발합니다.

이미 타격이 연결된 프레임에서 정확히 시작하세요. 펀치가 더 강하게 밀어붙이며 Tina를 옆으로 비틀어 날리고, 잔해가 바깥쪽으로 폭발합니다. Tina는 맹렬하고 우스꽝스러운 에너지로 튕겨 나와 접이식 의자를 휘둘러 위치를 잡고 “싸구려 기습이네, 해골 자식아!”라고 외칩니다. 이어서 거칠고 엉망인 반격으로 의자를 위로 휘두르며 “가구도 가져왔지!”라고 외칩니다. 의자가 복서에게 부딪히면서 솜이 카메라 쪽으로 폭발하고, 핑크 로프가 흔들리며, 관중이 폭발적으로 환호하고, 샷은 완전한 혼돈의 아수라장 속에서 끝납니다.

움직임: 빠른 핸드헬드 카메라, 공격적인 푸시인, 충격 흔들림, 휩처럼 급격한 리프레이밍, 로프 진동, 날아가는 솜, 흔들리는 의자, 과장된 인형 물리, 잔혹한 슬랩스틱 에너지.

오디오: 포효하는 인형 관중, 짧고 명확한 립싱크, 펀치 충격음, 의자 부딪히는 소리, 로프가 튕기는 소리, 솜이 터지는 소리, 경기장 저음 충격음, 혼란스러운 관중 반응.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
10s / image-to-video / one continuous shot / native audio / 16:9

Use the attached image as the exact first frame. Preserve the PuppetFight arena, hot-pink ring ropes, screaming puppet crowd, harsh overhead spotlights, gritty ring floor, exploding stuffing, and chaotic underground fight energy. Tina Tornado is on the left, airborne, wild hair and pink curlers flying, gripping a busted folding chair. On the right, the skull-gloved punk boxer in black sunglasses drives a savage punch into her as fabric scraps, dust, and stuffing burst through the air.

Start exactly on the frame with the hit already connecting. The punch drives harder and sends Tina twisting sideways while debris explodes outward. She snaps back with fierce, ridiculous energy, whipping the folding chair into position and yelling, “Cheap shot, skull boy!” She then swings the chair upward in a wild, messy counterattack and shouts, “I brought furniture!” The chair clips the boxer as stuffing blasts toward camera, the pink ropes shake, the crowd erupts, and the shot ends in full chaotic mayhem.

Motion: fast handheld camera, aggressive push-in, impact shakes, whip-like reframing, rope vibration, flying stuffing, chair wobble, exaggerated puppet physics, brutal slapstick energy.

Audio: roaring puppet crowd, short clear lip sync, punch impacts, chair clanks, rope twangs, stuffing pops, arena bass hit, chaotic crowd reaction.
```

</details>

#### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2062228478255116288/vid/avc1/1920x1080/ibkEHfzs5STuofwh.mp4?tag=27"><img src="https://pbs.twimg.com/amplify_video_thumb/2062228478255116288/img/iKuXlfkdwHozEcZp.jpg" height="420" alt="동기화된 슬랩스틱 오디오를 곁들인 원테이크 인형 싸움 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2062228478255116288/vid/avc1/1920x1080/ibkEHfzs5STuofwh.mp4?tag=27)**
</div>

#### 근거

- **제작자:** [Krista Gable](https://x.com/DomainDomme)
- **정식 출처:** [정식 출처](https://x.com/DomainDomme/status/2062228580851974229)
- **게시일:** 2026년 6월 3일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-11"></a>

### #4: 다섯 개의 타임드 광고 샷으로 보여 주는 럭셔리 향수

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Featured](https://img.shields.io/badge/Featured-gold-gold)

#### 이 워크플로가 중요한 이유

매크로 유리 디테일에서 꽃잎, 액체 움직임, 분무된 증기, 최종 히어로 구도로 이어지는 10초 제품 영상입니다.

#### 현지화 프롬프트

```
10초 시네마틱 럭셔리 향수 광고, 초사실적, 16:9, 하이패션 광고 스타일입니다.

장면: 어두운 분위기의 우아한 스튜디오. 공기 중에는 부드러운 안개가 있고, 극적인 스포트라이트 빔이 어둠을 가릅니다. 미니멀한 럭셔리 디자인과 눈에 보이는 우아한 브랜드 라벨을 가진 고급 유리 향수병이 중앙에 놓여 있습니다. 수정처럼 투명한 유리와 내부의 황금빛 액체, 초현실적인 반사를 표현하세요.

0–2초: 향수병을 극단적으로 클로즈업한 매크로 샷. 빛이 유리 가장자리를 따라 미끄러지고, 황금빛 액체가 반짝이며, 시네마틱 그림자와 하이엔드 패션 광고 조명을 사용합니다.

2–4초: 슬로 모션. 섬세한 꽃잎, 장미와 재스민이 병 주위에 나타나 회전하고, 부드러운 안개가 흐르며, 빛의 기둥 안에서 입자가 빛납니다.

4–6초: 병 안의 액체가 살아 있는 것처럼 부드럽게 움직이고, 황금빛 입자가 솟아오르며, 뚜껑이 슬로 모션으로 살짝 들어 올려집니다. 미묘한 마법적 리얼리즘 효과를 적용합니다.

6–8초: 향수 분사가 슬로 모션으로 공중에 폭발하고, 흐르는 실크 같은 증기와 꽃 형태로 변합니다. 우아하고 초현실적인 시네마틱 뷰티 샷입니다.

8–10초: 최종 히어로 샷. 반사 표면 중앙에 병을 놓고, 부드럽게 빛나는 안개와 꽃잎으로 둘러싸세요. 극적인 스포트라이트, 럭셔리 브랜드 집중, 하이패션 광고 엔딩을 사용합니다.

초사실적, 시네마틱 조명, 럭셔리 광고, 매크로 디테일, 유리 반사, 슬로 모션, 피사계 심도, 부드러운 빛, 필름 품질, 우아함, 프리미엄 미학, 텍스트 오버레이 없음, 자막 없음.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
10-second cinematic luxury perfume commercial, photorealistic, 16:9, high-fashion advertisement style.

Scene: a dark elegant studio with glossy black reflective surface, soft mist in the air, and dramatic spotlight beams cutting through the darkness.
A premium glass perfume bottle with a minimalist luxury design and a visible elegant brand label sits at the center, crystal-clear glass, golden liquid inside, ultra realistic reflections.

0–2s: extreme close-up macro shot of the perfume bottle, light glides across the glass edges, golden liquid shimmering, cinematic shadows, high-end fashion ad lighting.

2–4s: slow motion — delicate flower petals (rose, jasmine) appear and swirl around the bottle, soft mist flowing, particles glowing in light beams.

4–6s: liquid inside the bottle gently moves as if alive, golden particles rise, cap lifts slightly in slow motion, subtle magical realism effect.

6–8s: perfume spray bursts into the air in slow motion, transforming into flowing silk-like vapor and floral shapes, elegant and surreal, cinematic beauty shot.

8–10s: final hero shot — bottle centered on reflective surface, surrounded by soft glowing mist and petals, dramatic spotlight, luxury brand focus, high-fashion commercial ending.

ultra realistic, cinematic lighting, luxury advertisement, macro detail, glass reflections, slow motion, depth of field, soft glow, film quality, elegant, premium aesthetic, no text overlay, no subtitles.
```

</details>

#### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2038574681377447936/vid/avc1/1300x720/yOgaI_BSFPBmI_5L.mp4?tag=24"><img src="https://pbs.twimg.com/amplify_video_thumb/2038574681377447936/img/e5gg6UBuoz2e7u6J.jpg" height="420" alt="다섯 개의 타임드 광고 샷으로 보여 주는 럭셔리 향수 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2038574681377447936/vid/avc1/1300x720/yOgaI_BSFPBmI_5L.mp4?tag=24)**
</div>

#### 근거

- **제작자:** [Mr Das](https://x.com/MrDasOnX)
- **정식 출처:** [정식 출처](https://x.com/MrDasOnX/status/2038574814378783159)
- **게시일:** 2026년 3월 30일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="community-prompt-cases"></a>

## 커뮤니티 비디오 프롬프트

> 출처 날짜와 편집 가치순.

<a id="workflow-camera-direction-shot-design"></a>

### 카메라 연출 및 샷 디자인 (3)

프레이밍, 카메라 경로, 블로킹, 페이싱, 공개 순서, 전환을 중심으로 구성한 샷 브리프입니다.

**추천 비디오 프롬프트**

- [빠른 우주선 디테일 몽타주와 하드 트랜지션](#prompt-1)

<a id="prompt-2"></a>

#### #1: 타임드 스토리 비트를 따라 달리는 초소형 카메라맨

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

타임스탬프, 크기 기준, 핸드헬드 트래킹, 사운드 큐, 깔끔한 히어로 샷 엔딩을 사용하는 네 비트 미니어처 추격 장면입니다.

##### 현지화 프롬프트

```
16:9의 15초 시네마틱 영상을 만드세요. 거대한 콘텐츠 크리에이터의 책상 위를 달리는 키 3인치 카메라맨의 시점입니다. 0–4초: 도시 건물처럼 보이는 거대한 키보드 키 사이를 질주합니다. 4–8초: 네온 포털처럼 느껴지는 빛나는 휴대폰 화면 아래로 미끄러집니다. 8–12초: 거대한 탑 같은 커피잔 주위를 돌며 위쪽을 촬영합니다. 12–15초: 노트 위로 뛰어 올라 카메라를 시청자에게 향하고, 책상 세계가 극적인 히어로 샷 속에서 멈춥니다. 따뜻한 햇빛, 사실적인 미니어처 스케일, 부드러운 핸드헬드 트래킹, 장난기 있는 크리에이터 에너지. 오디오: 작은 발소리, 카메라 클릭음, 부드러운 휙 소리, 경쾌한 일렉트로닉 비트. 텍스트 없음, 자막 없음, 로고 없음.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Create a 15 second cinematic video in 16:9. From the POV of a tiny 3 inch cameraman running across a giant content creator’s desk. 0–4s: he sprints past huge keyboard keys like city buildings. 4–8s: he slides under a glowing phone screen that feels like a neon portal. 8–12s: he circles a coffee cup like a massive tower while filming upward. 12–15s: he jumps onto a notebook, points the camera at the viewer, and the desk world freezes in a dramatic hero shot. Warm sunlight, realistic miniature scale, smooth handheld tracking, playful creator energy. Audio: tiny footsteps, camera clicks, soft whooshes, upbeat electronic beat. No text, no subtitles, no logos.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2073857585468592128/vid/avc1/1280x720/rxriKyCHdrthsjfO.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2073857585468592128/img/ejoH2fAT7J9Pt6Xk.jpg" height="420" alt="타임드 스토리 비트를 따라 달리는 초소형 카메라맨 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2073857585468592128/vid/avc1/1280x720/rxriKyCHdrthsjfO.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [BMX](https://x.com/bmx_ai13)
- **정식 출처:** [정식 출처](https://x.com/bmx_ai13/status/2073857648848716232)
- **게시일:** 2026년 7월 5일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-18"></a>

#### #2: 밤비 뒤 소나무 물방울과 깃털을 터는 새

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

솔잎 물방울, 깃털 움직임, 얕은 심도와 절제된 비 갠 뒤 환경음을 조율한 조용한 세로형 자연 영상.

##### 현지화 프롬프트

```
부드러운 비가 밤새 내린 다음 아침. 흐린 새벽빛 속 젖은 소나무 가지를 담고, 솔잎 끝에 맺힌 투명한 물방울이 하나씩 떨어진다. 알록달록한 작은 새들이 가지에 앉아 짧고 자연스럽게 깃털의 빗물을 턴다. 느린 근접 트래킹, 얕은 심도, 사실적인 깃털과 물리, 차가운 숲 색감, 옅은 안개와 연속된 배경. 네이티브 오디오: 물방울, 가벼운 날갯짓, 먼 새소리, 솔숲 바람. 9:16 세로, 음악·문자·급격한 카메라 이동·중복 새 없음.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Morning after a full night of gentle rain. Frame wet pine branches in soft overcast dawn light as clear droplets gather on the needles and fall one by one. Colorful small birds perch among the branches, then shake rain from their feathers in brief natural bursts. Use a slow close tracking move, shallow depth of field, realistic feather and water physics, cool forest color, faint mist, and continuous background geography. Native audio: isolated drips, soft wing rustle, distant birdsong, and light wind through pine. Vertical 9:16, no music, no text, no sudden camera moves, no duplicate birds.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2079925362847526912/vid/avc1/720x1280/AdhMGM3sP3wytE_T.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2079925362847526912/img/LVL54rSZBBFdjw2e.jpg" height="420" alt="밤비 뒤 소나무 물방울과 깃털을 터는 새 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2079925362847526912/vid/avc1/720x1280/AdhMGM3sP3wytE_T.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [Lindy Hawkinson](https://x.com/HawkinsonLindy)
- **정식 출처:** [정식 출처](https://x.com/HawkinsonLindy/status/2079925422238937376)
- **게시일:** 2026년 7월 22일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="workflow-dialogue-performance-native-audio"></a>

### 대사, 연기 및 네이티브 오디오 (4)

대사, 연기, 분위기음, 음악 또는 동기화된 사운드가 장면을 이끄는 퍼포먼스 중심 프롬프트입니다.

**추천 비디오 프롬프트**

- [대사와 사운드 디자인을 곁들인 슬로 모션 시리얼 폭발](#prompt-3)
- [동기화된 슬랩스틱 오디오를 곁들인 원테이크 인형 싸움](#prompt-5)

<a id="prompt-4"></a>

#### #3: 멈춤과 미세 표정으로 이끄는 미묘한 말다툼

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

간결한 대사, 얼굴의 뉘앙스, 렌즈, 조명, 의도적인 멈춤을 통해 감정 연출을 시험하는 절제된 퍼포먼스 브리프입니다.

##### 현지화 프롬프트

```
분노한 커플의 감정적인 장면을 만드세요. 제한적이고 강렬한 대사, 적절하고 미묘한 미세 표정, 키 라이트와 보조광을 사용한 따뜻한 색보정, 아나모픽 렌즈를 적용하고 화면 텍스트는 넣지 마세요. 감정적으로 복잡하지만 미묘하고 적절한 앵글, 인디 영화 드라마의 감성을 지닌 젊은 미국인 커플, 분노와 멈춤을 통해 감정의 전 범위를 표현하세요.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Create an emotional scene of an angry couple, limited punchy dialogue, appropriate subtle micro expressions, warm color grading with key fill light and anamorphic lensing no onscreen text, emotionally complex but subtle proper angles, younger American couple with indie film drama sensibility with anger and pauses for a full range
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2070218111413035009/vid/avc1/1280x720/I6Uw_IbRpPre2CpL.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2070218111413035009/img/K5v2YoeYSUocr6rw.jpg" height="420" alt="멈춤과 미세 표정으로 이끄는 미묘한 말다툼 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2070218111413035009/vid/avc1/1280x720/I6Uw_IbRpPre2CpL.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [Kiri](https://x.com/Kyrannio)
- **정식 출처:** [정식 출처](https://x.com/Kyrannio/status/2070218164609405380)
- **게시일:** 2026년 6월 25일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-17"></a>

#### #4: 뉴욕을 질주하는 토끼

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

한 문장으로 피사체 움직임, 도시 연속성, 동기화 오디오를 시험하는 최소 공식 예제입니다.

##### 현지화 프롬프트

```
토끼 한 마리가 뉴욕시를 전력 질주한다. 거리 높이에서 역동적으로 추적하며 차량과 보행자의 일관성, 사실적인 발소리, 끊김 없는 도시 원근을 유지한다.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
A rabbit sprinting through New York City. Track it at street level with energetic forward motion, coherent traffic and pedestrian ambience, realistic footfalls, and continuous urban perspective.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2062331873305759744/vid/avc1/1280x720/xqRipmxEF1bG32eX.mp4?tag=14"><img src="https://pbs.twimg.com/amplify_video_thumb/2062331873305759744/img/xIyH77VNCyxnAPUR.jpg" height="420" alt="뉴욕을 질주하는 토끼 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2062331873305759744/vid/avc1/1280x720/xqRipmxEF1bG32eX.mp4?tag=14)**
</div>

##### 근거

- **제작자:** [@vercel_dev](https://x.com/vercel_dev)
- **정식 출처:** [정식 출처](https://x.com/vercel_dev/status/2062331926296641565)
- **게시일:** 2026년 6월 4일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="workflow-product-motion-commercial-spots"></a>

### 제품 모션 및 광고 스팟 (2)

제품, 오퍼, 의상, 요리, 기기 또는 브랜드 순간을 움직임의 중심에 두는 광고 클립입니다.

**추천 비디오 프롬프트**

- [다섯 개의 타임드 광고 샷으로 보여 주는 럭셔리 향수](#prompt-11)

<a id="prompt-12"></a>

#### #5: 자연스러운 녹차 따르기와 제품 엔드 카드

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

느린 따르기, 증기와 잎의 움직임, 따뜻한 일광, 자연의 분위기음, 음악, 패키지, 타임드 메시지 시퀀스로 구성한 웰니스 광고입니다.

##### 현지화 프롬프트

```
신선한 녹차를 투명한 유리잔에 슬로 모션으로 따르는 시네마틱 광고입니다. 증기가 부드럽게 피어오르고, 신선한 녹차 잎이 잔 주변으로 떨어집니다. 창가 근처의 나무 테이블 사이로 아침 햇살이 비쳐 따뜻하고 자연스러운 빛을 만듭니다. 찻잎에 맺힌 이슬방울의 클로즈업 샷, 새가 지저귀는 평화로운 자연 배경, 부드럽고 편안한 음악, 마지막에 다음 문구가 표시된 프리미엄 제품 패키지를 보여 주세요: ‘Pure Green Tea – Refresh Your Mind, Boost Your Day.’ 초사실적인 4K 광고 스타일입니다.

화면 텍스트 시퀀스:
0–5초: 상쾌하게 하루를 시작하세요
5–10초: 100% 천연 녹차
10–15초: 웰니스를 한 모금, 더 나은 삶을 사세요.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
A cinematic advertisement of fresh green tea being poured into a transparent glass cup in slow motion. Steam rises softly while fresh green tea leaves fall around the cup. Morning sunlight shines through a wooden table near a window, creating a warm natural glow. Close-up shots of dew drops on tea leaves, a peaceful nature background with birds chirping, soft relaxing music, premium product packaging displayed at the end with text: ‘Pure Green Tea – Refresh Your Mind, Boost Your Day.’ Ultra-realistic, 4K commercial style.

On-screen text sequence:
0–5 sec: Start Your Day Fresh
5–10 sec: 100% Natural Green Tea
10–15 sec: Sip Wellness, Live Better.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/ext_tw_video/2054166808329080832/pu/vid/avc1/1280x720/A7qudxSoiMyyGMbz.mp4?tag=25"><img src="https://pbs.twimg.com/ext_tw_video_thumb/2054166808329080832/pu/img/uMj-fcc76z8yMPzM.jpg" height="420" alt="자연스러운 녹차 따르기와 제품 엔드 카드 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/ext_tw_video/2054166808329080832/pu/vid/avc1/1280x720/A7qudxSoiMyyGMbz.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [Ur Meer](https://x.com/UrMeer289)
- **정식 출처:** [정식 출처](https://x.com/UrMeer289/status/2054167234290082149)
- **게시일:** 2026년 5월 12일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="workflow-image-to-video-subject-continuity"></a>

### 이미지-투-비디오 및 주제 연속성 (4)

정지 이미지를 애니메이션화하면서 인물의 정체성, 구도, 제품 형태 또는 스토리보드 레이아웃을 보존하는 이미지 기반 워크플로입니다.

<a id="prompt-6"></a>

#### #6: 실사 영화 연속성을 살린 횃불빛 판타지 키스

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Reference](https://img.shields.io/badge/Reference-Image%20Needed-orange)

##### 이 워크플로가 중요한 이유

의상, 세트 질감, 캐릭터 블로킹을 보호하면서 한 번의 실사 샷 안에서 코믹한 로맨틱 비트를 쌓는 레퍼런스 프레임 연속 장면입니다.

##### 현지화 프롬프트

```
빈티지 1960년대 실사 영화 제작 미학입니다. 업로드한 이미지를 14초짜리 실제 실사 영화 샷의 정확한 시작 프레임으로 사용하세요. 금발의 갑옷을 입은 여주인공과 키가 작고 거친 남자 배우가 횃불이 밝혀진 돌방 안에서 함께 프레임에 남아 있습니다. 젖은 머리카락은 불빛에 반짝이고, 실용적인 가죽과 금속 갑옷에는 마모와 녹청이 보이며, 연기가 공간을 떠돌고, 빛나는 불씨가 위로 떠오르고, 따뜻한 호박색과 주황색 불빛이 거칠게 다듬은 돌벽에 깜빡이는 그림자를 드리웁니다.

자연스러운 게이트 위브와 미묘한 필름 그레인이 있는 핸드헬드 35mm 아나모픽 카메라가 두 사람을 향해 천천히 푸시인합니다. 그녀는 교활하고 다정한 미소가 얼굴에 퍼지는 가운데 몸을 숙이고, 시선은 그의 눈에 고정합니다. 그는 그녀를 올려다보며 얼떨떨한 표정과 긴장된 기색을 보이고, 완전히 매료된 채 그 순간 얼어붙습니다.

그녀가 짧고 장난스러운 키스를 한 번 합니다. 빠르고 다정한 순간입니다. 바로 뒤에 그는 우스꽝스럽게 뒤로 기절하며 안전하게 구르듯 프레임 밖으로 사라집니다. 그녀는 몸을 곧게 세우고 조그만 승리의 미소를 지은 채, 즐거운 만족감으로 시선을 아래로 굴러간 그를 따라갑니다.

중세 판타지 영화의 가공되지 않은 실사 촬영본처럼 포착하세요. 풍부하고 따뜻한 색조를 가진 진짜 1960년대 필름 스톡의 모습, 자연스러운 결함, 횃불빛에서 생기는 약한 렌즈 플레어, 유기적인 카메라 흔들림을 사용하세요. 전체 시퀀스는 고전 모험 영화와 비슷한 연극적 블로킹과 타이밍으로 실시간 전개됩니다.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Vintage 1960s practical filmmaking aesthetic. Use the uploaded image as the exact start frame for a 14-second real live-action film shot: the blonde armored heroine and short rugged actor remain in frame together inside a torchlit stone chamber with wet hair glistening in firelight, practical leather and metal armor showing wear and patina, smoke drifting through the space, glowing embers floating upward, warm amber and orange firelight casting flickering shadows on rough-hewn stone walls.

Handheld 35mm anamorphic camera with natural gate weave and subtle film grain slowly pushes in on the pair. She leans down with a sly, affectionate smile spreading across her face, her eyes locked on his. He looks up at her, stunned expression, nervous energy, completely charmed and frozen in the moment.

She gives him one brief, playful kiss — a quick, tender moment. Immediately after, he comically faints backward, safely tumbling out of frame. She straightens up, holding a tiny victorious smirk, eyes following him down with amused satisfaction.

Captured like raw practical footage from a medieval fantasy movie — authentic 1960s film stock look with rich warm tones, natural imperfections, slight lens flares from torchlight, organic camera shake. The entire sequence unfolds in real time with theatrical blocking and timing reminiscent of classic adventure cinema.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2064435062935023616/vid/avc1/1280x720/6a89qw_0P05Eizwm.mp4?tag=27"><img src="https://pbs.twimg.com/amplify_video_thumb/2064435062935023616/img/2jonm-Korb5XcrU3.jpg" height="420" alt="실사 영화 연속성을 살린 횃불빛 판타지 키스 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2064435062935023616/vid/avc1/1280x720/6a89qw_0P05Eizwm.mp4?tag=27)**
</div>

##### 근거

- **제작자:** [Brent Lynch](https://x.com/BrentLynch)
- **정식 출처:** [정식 출처](https://x.com/BrentLynch/status/2064437767963332901)
- **게시일:** 2026년 6월 9일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-7"></a>

#### #7: 불꽃, 천, 오디오 물리학을 동반한 초고속 드래곤 비행

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Reference](https://img.shields.io/badge/Reference-Image%20Needed-orange)

##### 이 워크플로가 중요한 이유

카메라 경로, 공기역학적 움직임, 움직이는 불빛, 입자 분위기, 동기화된 사운드를 명시적인 제어 블록으로 분리한 고난도 레퍼런스 애니메이션입니다.

##### 현지화 프롬프트

```
이 레퍼런스 이미지를 HBO Game of Thrones와 House of the Dragon의 장대한 드래곤 시퀀스와 정확히 같은 시각적 스타일의 숨 막히는 10–12초 영상으로 충실하게 애니메이션화하세요. 레퍼런스 이미지와 완벽하게 일관성을 유지하세요. Daenerys의 외모, Drogon의 해부학적 구조, 비늘, 날개 구조, 초기 조명을 보존하세요.

Drogon이 King's Landing의 지붕 위를 낮게, 빠른 속도로 날아가며 강력하고 사실적인 날갯짓과 몸의 물결치는 움직임을 보입니다. 거대한 난류의 불꽃 숨결이 턱에서 뿜어져 나오고, 불꽃은 바람과 움직임에 역동적으로 반응하며 사실적인 유체 물리를 따릅니다. 빛나는 불씨가 뒤쪽으로 날아가고, 강한 열 왜곡과 빛 번짐이 나타납니다. 불빛은 아래에서 고대 석조 건물과 Red Keep을 극적으로 비추며, 움직이는 따뜻한 하이라이트와 깊은 그림자를 만듭니다.

Daenerys는 지휘하는 듯한 자세로 앞으로 몸을 기울입니다. 은빛 머리카락과 무거운 망토가 고속의 바람에 격렬하게 휘날리며 사실적인 천의 역학과 관성을 보입니다. 공중에는 미세한 불꽃과 재 입자가 떠돕니다.

카메라: Drogon을 약간 뒤쪽이자 측면에서 따라가는 역동적인 로우 앵글 시네마틱 트래킹 샷입니다. 카메라는 드래곤과 함께 고속으로 움직입니다. 드래곤의 움직임에 맞춰 미묘하게 상승하고 뱅킹하며 강력한 속도감, 규모감, 몰입감을 만듭니다. 통제된 시네마틱 정밀함에 약간의 핸드헬드 에너지를 섞은 쓸어가는 유연한 카메라 움직임입니다.

물리학: 매우 사실적인 드래곤 날개 막의 휘어짐과 바람을 받는 움직임, 자연스럽게 움직이는 각각의 날개 손가락, 진짜 무게와 난류를 가진 무거운 망토와 머리카락, 공기 움직임과 상호작용하는 실제 유체 역학에 따른 불꽃을 표현하세요.

조명 및 분위기: 드래곤의 불이 주된 이동 광원이며 아래에서 도시 건축물을 극적으로 밝힙니다. 체적 연기, 불씨, 열 아지랑이를 포함하세요. 도시에 약한 안개를 더해 장대한 대기 원근감을 만드세요.

네이티브 동기화 오디오: 강력하고 깊은 드래곤 포효에 격렬한 불꽃의 포효하는 휙 소리, 거센 바람 소리, 자연스러운 잔향이 있는 먼 도시 분위기음을 섞으세요. 포토리얼리스틱 렌더링, 일관된 움직임, 정교한 디테일, 아티팩트 없음, ARRI Alexa 수준의 충실도로 촬영한 영상. 걸작, 최대한 장대한 규모와 속도, 시네마틱 충격을 구현하세요.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Faithfully animate this reference image into a breathtaking cinematic 10-12 second video in the exact visual style of HBO Game of Thrones and House of the Dragon epic dragon sequences. Maintain perfect consistency with the reference image — Daenerys' appearance, Drogon's anatomy, scales, wing structure, and initial lighting.

Drogon flies at high speed low over King's Landing rooftops with powerful, realistic wing flaps and body undulation. Massive turbulent fire breath erupts from its jaws, flames reacting dynamically to wind and movement with realistic fluid physics, glowing embers flying backward, intense heat distortion and light bloom. Fire dramatically illuminates the ancient stone buildings and Red Keep from below with shifting warm highlights and deep shadows.

Daenerys leans forward with commanding posture, her silver hair and heavy cloak whipping violently in the high-speed wind with realistic fabric dynamics and inertia. Subtle sparks and ash particles in the air.

Camera: Dynamic low-angle cinematic tracking shot that follows Drogon from a slightly behind and side position, moving at high speed with the dragon. The camera subtly rises and banks with the dragon's movement, creating a powerful sense of speed, scale and immersion. Sweeping, fluid camera motion with slight handheld energy mixed with controlled cinematic precision.

Physics: Highly realistic dragon wing membrane flexing and catching the wind, individual wing fingers moving naturally, heavy cloak and hair with authentic weight and turbulence, fire behaving with real fluid dynamics and interaction with air movement.

Lighting & atmosphere: Dragon fire provides the primary moving light source, dramatically lighting the city architecture from below. Volumetric smoke, embers and heat haze. Epic atmospheric depth with slight haze over the city.

Native synchronized audio: Deep powerful dragon roar mixed with the roaring whoosh of intense fire, strong wind rush, and distant city ambience with natural reverb. Photorealistic rendering, coherent motion, intricate detail, no artifacts, shot with ARRI Alexa-level fidelity. Masterpiece, maximum epic scale, speed, and cinematic impact.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2067227819680178176/vid/avc1/1920x1080/Lyhy1DwxKbjXxXp-.mp4?tag=28"><img src="https://pbs.twimg.com/amplify_video_thumb/2067227819680178176/img/PAqDHuEWD_nCrT3q.jpg" height="420" alt="불꽃, 천, 오디오 물리학을 동반한 초고속 드래곤 비행 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2067227819680178176/vid/avc1/1920x1080/Lyhy1DwxKbjXxXp-.mp4?tag=28)**
</div>

##### 근거

- **제작자:** [AYi](https://x.com/AYi_AInotes)
- **정식 출처:** [정식 출처](https://x.com/AYi_AInotes/status/2067227900399562775)
- **게시일:** 2026년 6월 17일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-8"></a>

#### #8: 독립적인 미세 움직임을 가진 고정 에디토리얼 그리드

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Reference](https://img.shields.io/badge/Reference-Image%20Needed-orange)

##### 이 워크플로가 중요한 이유

8개 패널 레이아웃과 카메라는 고정하면서 호흡, 눈 깜빡임, 연기, 불꽃, 머리카락의 움직임을 제어하는 구조화된 안정성 실험입니다.

##### 현지화 프롬프트

```
{
  "video_generation": {
    "model": "grok",
    "duration": "6s",
    "aspect_ratio": "4:5",
    "motion_intensity": "low_realistic",
    "camera_motion": "none",

    "grid_lock": true,
    "no_zoom": true,
    "no_crop": true,
    "no_panel_shift": true,

    "identity_lock": {
      "preserve_face": true,
      "preserve_hands": true,
      "preserve_glove": true,
      "preserve_cigarette_shape": true,
      "preserve_background": true
    },

    "scene": "8패널의 거친 에디토리얼 그리드. 젖은 머리의 여성, 스모키 아이, 검은 장갑, 중립적인 벽. 강한 플래시 조명. 패널은 완벽하게 고정된 상태로 유지됩니다.",

    "movement_rules": {
      "start_immediately": true,
      "no_slow_motion": true,
      "natural_speed": true,
      "all_panels_active": true,

      "breathing": "모든 패널에서 눈에 보이는 자연스러운 호흡",
      "blink": "패널 전체에 분산된 자연스러운 눈 깜빡임",
      "eye_movement": "연속적인 미세 눈동자 이동",
      "smoke": "물리적으로 정확하고 지속적으로 위로 흐르는 연기",
      "flame": "지속적이고 미묘한 불꽃 일렁임",
      "hair": "젖은 머리카락 가닥의 가벼운 움직임"
    },

    "stability_rules": [
      "카메라 이동 없음",
      "레이아웃 변경 없음",
      "얼굴 변형 없음",
      "입술 떨림 없음",
      "손가락 추가 없음",
      "연기 결함 없음",
      "배경 깜빡임 없음"
    ],

    "quality_mode": "max_stability",

    "negative_prompt": [
      "슬로 모션",
      "정지 화면",
      "줌",
      "크롭",
      "패널 드리프트",
      "뒤틀린 얼굴",
      "AI 흔들림"
    ]
  }
}
```

<details>
<summary>원문 출처 프롬프트</summary>

```
{
  "video_generation": {
    "model": "grok",
    "duration": "6s",
    "aspect_ratio": "4:5",
    "motion_intensity": "low_realistic",
    "camera_motion": "none",

    "grid_lock": true,
    "no_zoom": true,
    "no_crop": true,
    "no_panel_shift": true,

    "identity_lock": {
      "preserve_face": true,
      "preserve_hands": true,
      "preserve_glove": true,
      "preserve_cigarette_shape": true,
      "preserve_background": true
    },

    "scene": "8-panel gritty editorial grid. Wet hair woman, smoky eyes, black glove, neutral wall. Hard flash lighting. Panels remain perfectly fixed.",

    "movement_rules": {
      "start_immediately": true,
      "no_slow_motion": true,
      "natural_speed": true,
      "all_panels_active": true,

      "breathing": "visible natural breathing in every panel",
      "blink": "natural blinks distributed across panels",
      "eye_movement": "continuous micro eye shifts",
      "smoke": "constant upward smoke flow, physically accurate",
      "flame": "continuous subtle flame flicker",
      "hair": "light wet strand motion"
    },

    "stability_rules": [
      "no camera movement",
      "no layout change",
      "no face morph",
      "no lip jitter",
      "no extra fingers",
      "no smoke glitch",
      "no background flicker"
    ],

    "quality_mode": "max_stability",

    "negative_prompt": [
      "slow motion",
      "freeze frame",
      "zoom",
      "crop",
      "panel drift",
      "warped face",
      "AI jitter"
    ]
  }
}
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2024901412547641344/vid/avc1/720x1280/aLY-ZGPl4g6vpthr.mp4?tag=21"><img src="https://pbs.twimg.com/amplify_video_thumb/2024901412547641344/img/RWFQtc5G2WzSqmXu.jpg" height="420" alt="독립적인 미세 움직임을 가진 고정 에디토리얼 그리드 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2024901412547641344/vid/avc1/720x1280/aLY-ZGPl4g6vpthr.mp4?tag=21)**
</div>

##### 근거

- **제작자:** [Özge Döner](https://x.com/astronomerozge1)
- **정식 출처:** [정식 출처](https://x.com/astronomerozge1/status/2024901494290743510)
- **게시일:** 2026년 2월 20일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-15"></a>

#### #9: 타임스스퀘어에 잠든 거인

![Language-EN](https://img.shields.io/badge/Language-EN-blue)
![Reference](https://img.shields.io/badge/Reference-Image%20Needed-orange)

##### 이 워크플로가 중요한 이유

인물을 도심에 잠든 거인으로 바꾸고 교통, 도로 표시, 전광판과 비스듬한 드론 시점으로 규모를 보여 주는 참조 기반 초현실 영상입니다.

##### 현지화 프롬프트

```
업로드한 얼굴을 참조해 붐비는 타임스스퀘어 교차로 한가운데 웅크려 잠든 거인으로 표현한 극단적 원근감의 드론 샷. 교통은 거대한 몸을 피해 흐르고 노란 택시는 장난감 자동차처럼 작아 보인다. 인물은 편안한 오버사이즈 회색 스웨트팬츠와 후디를 입고 있다. 아스팔트의 노면 표시와 거리의 작은 잔해가 크기 비교 기준을 만든다. 밤의 타임스스퀘어 전광판이 뿜는 화려한 전기빛이 잠든 인물 위로 여러 색을 드리운다. 카메라는 위에서 내려다보되 건물 높이가 드러나도록 살짝 기울인다. 8K 해상도, 사실적인 군중과 차량 디테일, 사적인 친밀함과 공공의 혼돈이 대비되는 초현실적 장면.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
An extreme perspective drone shot capturing the uploaded face as reference as a sleeping giant curled up in the middle of a busy Times Square intersection. Traffic is diverted around the giant body, with yellow taxis appearing as toy cars in comparison. The subject is wearing oversized, comfortable grey sweatpants and a hoodie. The asphalt markings and street debris provide scale cues. The lighting is the electric vibrance of Times Square billboards at night, casting multi-colored light across the sleeping figure. The camera angle is top-down but slightly angled to show the height of the buildings. 8k resolution, photorealistic crowd and vehicle details, and a surreal juxtaposition of intimacy and public chaos.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2077442928272953344/vid/avc1/832x1504/-WXJulilhZsyGpHg.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2077442928272953344/img/I7WDDHxses1etiXw.jpg" height="420" alt="타임스스퀘어에 잠든 거인 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2077442928272953344/vid/avc1/832x1504/-WXJulilhZsyGpHg.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [Mr Das](https://x.com/MrDasOnX)
- **정식 출처:** [정식 출처](https://x.com/MrDasOnX/status/2077443101631926682)
- **게시일:** 2026년 7월 15일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="workflow-stylized-motion-visual-effects"></a>

### 스타일라이즈드 모션 및 시각 효과 (4)

변형, 시뮬레이션, 초현실적 물리, 그래픽 모션 또는 독특한 미디어 처리로 구동되는 효과 및 애니메이션 패턴입니다.

<a id="prompt-9"></a>

#### #10: 절제된 슬로 모션으로 달빛 아래 코를 비비는 드래곤

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

하나의 다정한 동작, 미묘한 근육 움직임, 반사되는 비늘 디테일, 부드러운 달빛 분위기에 집중하는 정적인 클로즈업입니다.

##### 현지화 프롬프트

```
시네마틱 클로즈업, 슬로 모션 비디오입니다. 거대하고 근육질인 황금색 드래곤과 별이 빛나는 생체 발광 비늘을 가진 더 가늘고 우아한 자정빛 보라색 드래곤이 달빛 어린 마법의 숲에서 쉬고 있습니다. 두 드래곤이 천천히 다정하게 머리를 서로에게 가져갑니다. 황금색 드래곤이 보라색 드래곤의 목을 부드럽게 코로 건드립니다. 두 드래곤의 비늘이 반짝이며 은빛 달빛을 반사합니다. 카메라 움직임: 정적인 샷으로, 비늘 아래의 미묘한 근육 물결과 눈에서 부드럽게 빛나는 호박색 광채를 포착합니다. 분위기 및 조명: 거대한 보름달이 있는 소프트 포커스 배경.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Cinematic close-up, slow-motion video. A massive, muscular golden dragon and a more slender, elegant midnight purple dragon with starry bioluminescent scales are resting in a moonlit enchanted forest. The two dragons slowly and tenderly move their heads toward each other. The golden dragon gently nudges the purple dragon’s neck. Their scales shimmer and reflect the silver moonlight. Camera Movement: static shot, The camera captures the subtle muscle ripples under the scales and the soft amber glow in their eyes. Atmosphere & Lighting: Soft-focus background with a giant full moon.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/ext_tw_video/2053639303700566016/pu/vid/avc1/1168x1728/iTKoCMbepTjPPd4a.mp4?tag=23"><img src="https://pbs.twimg.com/ext_tw_video_thumb/2053639303700566016/pu/img/paFojbfcBAr6mUm_.jpg" height="420" alt="절제된 슬로 모션으로 달빛 아래 코를 비비는 드래곤 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/ext_tw_video/2053639303700566016/pu/vid/avc1/1168x1728/iTKoCMbepTjPPd4a.mp4?tag=23)**
</div>

##### 근거

- **제작자:** [auqib](https://x.com/auqibhabib)
- **정식 출처:** [정식 출처](https://x.com/auqibhabib/status/2053639419266322623)
- **게시일:** 2026년 5월 11일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-10"></a>

#### #11: 강한 충격 물리학을 적용한 연속 무술 이동

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

지형 이동, 연속 전투 동작, 사실적인 운동량, 원형 카메라 움직임, 무편집 제약을 결합한 6초 액션 브리프입니다.

##### 현지화 프롬프트

```
걸작 수준의 6초 영상. 숙련도가 매우 높은 전사가 잔혹하고 강도 높은 싸움을 벌이며 거친 지형을 가로지릅니다. 캐릭터는 회전 발차기, 플라잉 니, 공중제비, 벽 달리기, 잔혹한 제압을 포함한 빠르고 강력한 무술 콤보를 수행합니다. 강한 충격과 운동량이 있는 사실적인 물리학을 적용하세요. 트래킹 샷, 원형 카메라, 극적인 로우 앵글을 사용하는 역동적인 카메라 워크. 시네마틱 조명, 선명한 움직임, 강렬하고 날것 같은 액션, 컷 없음, 부드럽고 강력한 안무, 최고 품질, 6초 영상.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Masterpiece quality, 6-second video of a highly skilled fighter traversing rough terrain while engaged in a brutal, high-intensity fight. The character performs fast, powerful martial arts combos including spinning kicks, flying knees, aerial flips, wall runs, and savage takedowns. Strong realistic physics with heavy impact and momentum. Dynamic camera work with tracking shots, circling camera, and dramatic low angles. Cinematic lighting, sharp motion clarity, intense and raw action, no cuts, smooth and powerful choreography, best quality, 6-second video.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2067298117415059456/vid/avc1/688x464/-y07rGLLEnox_zTy.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2067298117415059456/img/fFgLi1L_hufsoKI6.jpg" height="420" alt="강한 충격 물리학을 적용한 연속 무술 이동 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2067298117415059456/vid/avc1/688x464/-y07rGLLEnox_zTy.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [⭕Kisalay](https://x.com/Kisalay_)
- **정식 출처:** [정식 출처](https://x.com/Kisalay_/status/2067298200520896673)
- **게시일:** 2026년 6월 17일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-14"></a>

#### #12: 역류해 떠오르는 물 구체

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

유리잔의 물이 위로 솟아나 부드러운 낮빛 아래 공중에 떠 있는 구체로 모이는, 물리 법칙을 거스르는 간결한 동영상 프롬프트입니다.

##### 현지화 프롬프트

```
테이블 위에 물이 든 유리잔이 있다. 갑자기 물이 잔 밖으로 위로 흐르며 공중에 떠 있는 구체를 만든다. 사실적 표현, 부드러운 낮빛
```

<details>
<summary>원문 출처 프롬프트</summary>

```
A glass of water on a table, the water suddenly flows upward out of the glass and forms a floating sphere, photorealistic, soft daylight
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2077061435562364928/vid/avc1/752x416/sWQUveRllw3DBHTI.mp4?tag=25"><img src="https://pbs.twimg.com/amplify_video_thumb/2077061435562364928/img/8qeVxVJrvKvXuW5h.jpg" height="420" alt="역류해 떠오르는 물 구체 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2077061435562364928/vid/avc1/752x416/sWQUveRllw3DBHTI.mp4?tag=25)**
</div>

##### 근거

- **제작자:** [atmen](https://x.com/atmen189)
- **정식 출처:** [정식 출처](https://x.com/atmen189/status/2077238726120669192)
- **게시일:** 2026년 7월 15일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="prompt-16"></a>

#### #13: 등불이 빛나는 JRPG 시장

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

붐비는 시장, 3인칭 카메라, 읽기 쉬운 UI, 옷감 물리와 대기 효과를 결합한 게임플레이 장면입니다.

##### 현지화 프롬프트

```
현대 JRPG의 시네마틱 게임플레이 영상. NPC와 상인으로 붐비는 시장, 따뜻한 등불, 3인칭 카메라, 볼류메트릭 광선과 파티클, 옷감 물리가 적용된 정교한 캐릭터, 사실적인 게임 엔진 텍스처, 피사계 심도와 모션 블러. 애니메이션풍 아트 디렉션과 환경 스토리텔링. HP/MP 바와 미니맵은 계속 보이게 유지합니다. 부드러운 60fps 카메라 움직임, 4K 화면.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Cinematic gameplay footage from a modern JRPG video game: a bustling marketplace filled with NPCs and vendors, warm lantern lighting, third-person camera angle, dynamic volumetric god rays, particle effects, detailed character models with cloth physics, photorealistic game-engine textures, depth of field, and motion blur. Anime-inspired art direction with atmospheric environmental storytelling. Keep UI elements visible, including HP/MP bars and a minimap. Smooth 60 fps camera movement, 4K presentation.
```

</details>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/1987063720108040194/vid/avc1/464x688/pa6Bp_3lB7jCXuMF.mp4?tag=23"><img src="https://pbs.twimg.com/amplify_video_thumb/1987063720108040194/img/VuuPeYmBCr09uM7y.jpg" height="420" alt="등불이 빛나는 JRPG 시장 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/1987063720108040194/vid/avc1/464x688/pa6Bp_3lB7jCXuMF.mp4?tag=23)**
</div>

##### 근거

- **제작자:** [@icreatelife](https://x.com/icreatelife)
- **정식 출처:** [정식 출처](https://x.com/icreatelife/status/1987063774449430763)
- **게시일:** 2025년 11월 8일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

<a id="workflow-video-editing-restyling-scene-control"></a>

### 비디오 편집, 스타일 변경 및 장면 제어 (1)

연속성을 보호하면서 기존 영상의 스타일을 바꾸고 확장, 추가, 제거, 교체하거나 장면의 방향을 바꾸는 워크플로입니다.

<a id="prompt-13"></a>

#### #14: 기존 클립의 원자적 사이버펑크 스타일 변경

![Language-EN](https://img.shields.io/badge/Language-EN-blue)

##### 이 워크플로가 중요한 이유

원본 비디오의 기본 움직임은 유지하면서 시각적 처리를 미래적인 사이버펑크 스타일로 바꿀 수 있음을 보여 주는 의도적으로 간결한 편집 지시입니다.

##### 현지화 프롬프트

```
스타일을 미래적인 Cyberpunk로 변경하세요.
```

<details>
<summary>원문 출처 프롬프트</summary>

```
Change the style to futuristic Cyberpunk
```

</details>

##### 출처 및 결과 프레임

<table>
<tr>
<td width="50%" valign="top" align="center"><img src="https://pbs.twimg.com/amplify_video_thumb/2018048438965768192/img/N0xaRAp2EYmuPcUF.jpg" width="100%" alt="기존 클립의 원자적 사이버펑크 스타일 변경 - Image 1"></td>
<td width="50%" valign="top" align="center"><img src="https://pbs.twimg.com/amplify_video_thumb/2018048888234553344/img/Gjbr2r7c4rig1ghx.jpg" width="100%" alt="기존 클립의 원자적 사이버펑크 스타일 변경 - Image 2"></td>
</tr>
</table>

##### 동영상

<div align="center">
<a href="https://video.twimg.com/amplify_video/2018048888234553344/vid/avc1/788x1168/_LiqbALuNiUVFZ9B.mp4?tag=21"><img src="https://pbs.twimg.com/amplify_video_thumb/2018048888234553344/img/Gjbr2r7c4rig1ghx.jpg" height="420" alt="기존 클립의 원자적 사이버펑크 스타일 변경 - Motion preview"></a>

*미리보기를 클릭하여 동영상을 여세요* · **[▶ 동영상 보기 →](https://video.twimg.com/amplify_video/2018048888234553344/vid/avc1/788x1168/_LiqbALuNiUVFZ9B.mp4?tag=21)**
</div>

##### 근거

- **제작자:** [Tyler Keehner](https://x.com/STFUnion)
- **정식 출처:** [정식 출처](https://x.com/STFUnion/status/2018049167839400312)
- **게시일:** 2026년 2월 1일
- **프롬프트 언어:** en

**[이 연출로 만들기 · ImagineVid](https://imaginevid.io/ko/grok-imagine)**

---

## 검증된 사례 기여하기

실제 연출 패턴을 가르치는 Grok Imagine Video 사례를 찾았나요? GitHub Issues를 통해 프롬프트, 재생 가능한 결과, 제작자, 출처, 모델 근거, 입력 모드를 제출하세요.

### GitHub issue

1. [**비디오 프롬프트 제출**](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills/issues/new?template=submit-prompt.yml)
2. 전체 브리프, 출처, 제작자, 모델 근거, 재생 가능한 미디어를 제공하세요
3. 메인테이너가 출처, 비디오 가치, 범위, 중복을 확인합니다
4. 승인된 사례는 로컬 데이터 소스로 정규화됩니다
5. 모든 품질 검사를 통과하면 생성기가 사례를 게시합니다

**편집 원칙:** 인기는 근거가 아닙니다. 완전한 프롬프트와 유용한 비디오가 있는 낮은 참여도의 게시물이 재현 가능한 연출이 없는 바이럴 쇼케이스보다 우선될 수 있습니다.

제출하기 전에 [CONTRIBUTING.md](docs/CONTRIBUTING.md)를 읽어 주세요.

---

## 라이선스

ImagineVid가 작성한 편집 문구와 코드는 [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)에 따라 라이선스됩니다. 제3자 프롬프트, 제작자 신원, 상표, 이미지, 비디오는 각 소유자에게 있으며 해당 라이선스에서 제외됩니다.

---

## 제작자 크레딧

<details>
<summary>Community creators we thank (16)</summary>

[@icreatelife](https://x.com/icreatelife) · [@vercel_dev](https://x.com/vercel_dev) · [⭕Kisalay](https://x.com/Kisalay_) · [atmen](https://x.com/atmen189) · [auqib](https://x.com/auqibhabib) · [AYi](https://x.com/AYi_AInotes) · [BMX](https://x.com/bmx_ai13) · [Brent Lynch](https://x.com/BrentLynch)<br>
[Dogan Ural](https://x.com/doganuraldesign) · [Kiri](https://x.com/Kyrannio) · [Krista Gable](https://x.com/DomainDomme) · [Lindy Hawkinson](https://x.com/HawkinsonLindy) · [Mr Das](https://x.com/MrDasOnX) · [Özge Döner](https://x.com/astronomerozge1) · [Tyler Keehner](https://x.com/STFUnion) · [Ur Meer](https://x.com/UrMeer289)

</details>

---

## 저장소 성장

[![GitHub stars](https://img.shields.io/github/stars/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills?style=for-the-badge&logo=github&label=GitHub%20Stars)](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills/stargazers)

**[저장소 성장](https://star-history.com/#imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills&Date)**

---

<div align="center">

**[Grok Imagine Video로 만들기](https://imaginevid.io/ko/grok-imagine)** •
**[검증된 사례 제출](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills/issues/new?template=submit-prompt.yml)** •
**[컬렉션에 별표하기](https://github.com/imaginevid-ai/Awesome-grok-imagine-video-prompts-and-skills)**

<sub>버전 관리된 로컬 데이터에서 생성된 시각 2026-07-23T10:20:48.809Z</sub>

</div>
